var objLoader = {
    ...
    handleLoadedObject: function( data ) {
        ...
        for ( i = 0; i < data.length; i++ ) {
            ...
            switch( temp[ 0 ] ) {
                case "v":
                    vertices = objLoader.pushData( vertices, temp );
                    break;
                case "vt":
                    textures = objLoader.pushData( textures, temp );
                    break;
                case "vn":
                    normals = objLoader.pushData( normals, temp );
                    break;
                case "f":
                    ...
                    temp = temp.join( " " );
                    temp = temp.split( " " );
                    faces = objLoader.pushData( faces, temp );
                    break;
                default:
                    break;
            }
